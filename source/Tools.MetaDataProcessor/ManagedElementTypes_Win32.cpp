...
string CLR_RT_ManagedElementType::GetMarshalCodeBeforeNativeCall(bool bStaticMethod, UINT32 prmInd)
{
	...
	strCode += string("        ") + "NANOCLR_CHECK_HRESULT( Interop_Marshal_" + GetNativeType() + "( stack, " + GetParamIndexAsText(prmInd + !bStaticMethod) + ", param" + strIndex + " ) );\n";
	return strCode;
}
...
string CLR_RT_ManagedElementTypeByRef::GetMarshalCodeBeforeNativeCall(bool bStaticMethod, UINT32 prmInd)
{
	...
	strCode += string("        ") + "NANOCLR_CHECK_HRESULT( Interop_Marshal_" + GetNativeType() + "_ByRef( stack, heapblock" + strIndex + ", " + GetParamIndexAsText(prmInd + !bStaticMethod) + ", param" + strIndex + " ) );\n";
	return strCode;
}
...
string CLR_RT_ManagedElementTypeArray::GetMarshalCodeBeforeNativeCall(bool bStaticMethod, UINT32 prmInd)
{
	...
	strCode += "        NANOCLR_CHECK_HRESULT( Interop_Marshal_" + GetNativeType() + "_ARRAY( stack, " + GetParamIndexAsText(prmInd + !bStaticMethod) + ", param" + strIndex + " ) );\n";
	return strCode;
}
...
string CLR_RT_ManagedElementTypeByRef::GetMashalCodeAfterNativeCall(UINT32 prmInd, bool bStaticMethod)
{
	...
	return string("        NANOCLR_CHECK_HRESULT( Interop_Marshal_StoreRef( stack, heapblock") + strBlockIndex + ", " + strSaveIndex + " ) );\n";
}
