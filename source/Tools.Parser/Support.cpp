...
HRESULT ErrorReporting::ConstructErrorOrigin( std::wstring &str, ISymUnmanagedReader* pSymReader, mdMethodDef md, ULONG32 ipOffset )
{
    ...
    if(!pSymReader) NANOCLR_SET_AND_LEAVE(CLR_E_FAIL);
    ...
    NANOCLR_CHECK_HRESULT(pMethod->GetSequencePoints( cSeqPoints, &cSeqPoints, pOffsets, pDocs, pLines, pCols, pEndLines, pEndCols ));
    ...
    for(ULONG32 i = 0; i < cSeqPoints; i++)
    {
        if(i == cSeqPoints || ipOffset < pOffsets[i+1])
        {
            ...
            if(pLines[i] == NO_SOURCE_AVAILABLE) NANOCLR_SET_AND_LEAVE(CLR_E_FAIL);
