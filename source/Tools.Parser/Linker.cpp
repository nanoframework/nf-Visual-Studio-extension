...
HRESULT WatchAssemblyBuilder::Linker::DumpDownloads(std::wstring szFileNamePE )
{
    NANOCLR_HEADER();
	...
	NANOCLR_CHECK_HRESULT(xml.New( L"SpotDownloadDescriptor" ));
    ...
    if(m_pr)
    {
		...
		NANOCLR_CHECK_HRESULT(xml.CreateNode( L"Assembly", &pNodeAssembly ));

		NANOCLR_CHECK_HRESULT(xml.PutValue( L"Name", m_pr->m_assemblyName, fFound, pNodeAssembly ));

		NANOCLR_CHECK_HRESULT(local_DumpVersion( xml, pNodeAssembly, L"Version", m_pr->m_version ));

        NANOCLR_CHECK_HRESULT(GetFullPath ( m_pr->m_assemblyFile, &strPath, NULL                  ));
		NANOCLR_CHECK_HRESULT(xml.PutValue( L"Source",             strPath, fFound, pNodeAssembly ));

        NANOCLR_CHECK_HRESULT(GetFullPath ( szFileNamePE,   &strPath, NULL                  ));
		NANOCLR_CHECK_HRESULT(xml.PutValue( L"Destination",  strPath, fFound, pNodeAssembly ));

        {
            IXMLDOMNodePtr pNodeClasses;

			NANOCLR_CHECK_HRESULT(xml.CreateNode( L"Publish", &pNodeClasses, pNodeAssembly ));
            ...
            for(MetaData::TypeDefMapIter itTypeDef = m_pr->m_mapDef_Type.begin(); itTypeDef != m_pr->m_mapDef_Type.end(); itTypeDef++)
            {
                ...
                if(MetaData::IsTokenPresent( m_pr->m_setAttributes_Types_PublishInApplicationDirectory, td.m_td ))
                {
					...
					NANOCLR_CHECK_HRESULT(xml.CreateNode( L"Class",              &pNodeClass, pNodeClasses ));
					NANOCLR_CHECK_HRESULT(xml.PutValue  ( L"Name" , str, fFound,  pNodeClass               ));
                }
            }
        }
    }
...

    {
		...
		NANOCLR_CHECK_HRESULT(xml.Save( strFile.c_str() ));
    }
